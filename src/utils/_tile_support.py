...
class TextRenderer:
    ...
    def render_text(self,screen,text,pos=(16,16),color = (150,150,150),topleft = False):
...
class Button:
    ...
    def clicking(self,button = 0):
        self.clicked = False
        if pygame.mouse.get_just_pressed()[button] and self.rect.collidepoint(pygame.mouse.get_pos()):
            self.clicked = True
...
def drawPolygons(surface,polygon,color = (255,255,255)):
    if not (len(polygon) > 1):
        pygame.draw.circle(surface,color,polygon[0],3)
        return
    for i,vertice in enumerate(polygon):
        if i == len(polygon)-1:
            pygame.draw.line(surface, color, (vertice.x,vertice.y), (polygon[0].x,polygon[0].y))
            break
        nextv = polygon[i+1]
        pygame.draw.line(surface, color, (vertice.x,vertice.y), (nextv.x,nextv.y))
